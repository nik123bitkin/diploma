...
using CompomentLib.SpectrumAnalyzer;
using CompomentLib.Timeline;
using Core.Entities;
using Core.Handlers;
using NAudio.Wave;
        ...
        public void OpenFile(string path)
        {
            ...
            if (System.IO.File.Exists(path))
            {
                try
                {
                    ...
                    inputStream = new WaveChannel32(ActiveStream, Equalizer);

                    sampleAggregator = new SampleAggregator(fftDataSize);
                    inputStream.Sample += InputStream_Sample;
                    channelCount = inputStream.WaveFormat.Channels;
